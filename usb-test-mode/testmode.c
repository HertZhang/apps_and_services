...
static void usage(char *cmd)
{
	fprintf(stdout, 
"	Usage:\n\
		1. lsusb (list the VID and PID of usb devices)\n\
		2. %s -D VID:PID -t testmode\n\
	Test mode:\n\
		test_packet, test_se0_nak, test_k, test_j,\n\
		test_force_hs, test_force_fs\n\
	Example:\n\
		./testmode -D V:P -t test_packet\n\
		./testmode -D V:P -t test_se0_nak\n\
		./testmode -D V:P -t test_k\n\
		./testmode -D V:P -t test_j\n\
		./testmode -D V:P -t test_force_hs\n\
		./testmode -D V:P -t test_force_fs\n"
	, cmd);
}
